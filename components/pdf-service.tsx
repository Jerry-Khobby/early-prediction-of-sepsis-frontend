...
export const generatePDF = async (reportData: ReportData, options: PDFOptions): Promise<Blob> => {
  ...
  addHeader(doc, reportData.reportType, reportData.reportId, reportData.generatedDate, primaryColor, secondaryColor, font)
  ...
  if (includeOptions.riskAssessment && reportData.risk_assessment && reportData.key_risk_factors) {
    yPos = addRiskAssessment(doc, reportData.risk_assessment, reportData.key_risk_factors, yPos, primaryColor, font)
  }

  // Add model explanation if selected and available
  if (includeOptions.modelExplanation && reportData.key_risk_factors) {
    yPos = addModelExplanation(doc, reportData.key_risk_factors, yPos, primaryColor, font)
  }
  ...
  if (includeOptions.customSections && reportData.custom_sections) {
    for (const sectionId of includeOptions.customSections) {
      const section = reportData.custom_sections.find(s => s.title === sectionId)
      if (section) {
...
function addHeader(
  ...
  doc.text(`Report ID: ${reportId}`, doc.internal.pageSize.getWidth() - 15, 10, {
    align: "right",
  })
  doc.text(`Generated: ${generatedDate}`, doc.internal.pageSize.getWidth() - 15, 15, {
    align: "right",
  })
}

// Helper function to add patient information
function addPatientInfo(doc: jsPDF, patientInfo: Record<string, any>, yPos: number, font: string): number {
...
function addSummary(doc: jsPDF, riskAssessment: RiskAssessment, yPos: number, font: string): number {
  if (yPos > doc.internal.pageSize.getHeight() - 60) {
    doc.addPage()
    yPos = 20
  }
...
function addRiskAssessment(
  ...
  const riskColor = riskScore >= 0.75 ? [239, 68, 68] : 
                    riskScore >= 0.5 ? [245, 158, 11] : 
                    [16, 185, 129]
  ...
  const sortedFactors = [...riskFactors].sort((a, b) => Math.abs(b.value) - Math.abs(a.value))
...
function addModelExplanation(
  ...
  sortedFeatures.forEach((feature, index) => {
    const normalizedValue = Math.abs(feature.value) / maxValue
    ...
    doc.setFillColor(...primaryColor)
    doc.rect(100, yPos - 4, 80 * normalizedValue, 5, "F")
...
function addRecommendations(
  ...
  // Diagnostic tests
  if (clinicalGuidance.diagnostic_tests && clinicalGuidance.diagnostic_tests.length > 0) {
    doc.setFontSize(12)
    doc.setFont(font, "bold")
  ...
  if (clinicalGuidance.treatment_options) {
    const { immediate_medications, antibiotic_choices } = clinicalGuidance.treatment_options
    ...
    if (antibiotic_choices) {
      ...
      if (antibiotic_choices.community_acquired && antibiotic_choices.community_acquired.length > 0) {
        doc.setFont(font, "bold")
      ...
      if (antibiotic_choices.hospital_acquired && antibiotic_choices.hospital_acquired.length > 0) {
        doc.setFont(font, "bold")
      ...
      if (antibiotic_choices.penicillin_allergy && antibiotic_choices.penicillin_allergy.length > 0) {
        doc.setFont(font, "bold")
  ...
  // Required actions
  if (clinicalGuidance.required_actions && clinicalGuidance.required_actions.length > 0) {
    doc.setFontSize(12)
    doc.setFont(font, "bold")
...
function addSafetyAlerts(doc: jsPDF, safetyAlerts: string[], yPos: number, font: string): number {
  if (yPos > doc.internal.pageSize.getHeight() - 60) {
    doc.addPage()
    yPos = 20
  }
  ...
  safetyAlerts.forEach((alert, index) => {
    ...
    if (alert.toLowerCase().includes('critical') || alert.toLowerCase().includes('urgent')) {
    ...
    const textLines = doc.splitTextToSize(alertText, doc.internal.pageSize.getWidth() - 30)
    doc.text(textLines, 15, yPos)
...
function addCustomSection(doc: jsPDF, section: {
  title: string
  content: string | string[]
  type: 'text' | 'list' | 'keyValue'
}, yPos: number, font: string): number {
  if (yPos > doc.internal.pageSize.getHeight() - 60) {
    doc.addPage()
    yPos = 20
  }
  ...
  if (section.type === 'text') {
    ...
    content.forEach(line => {
      ...
      if (key && value) {
        ...
        doc.setFont(font, "normal")
        doc.text(value.trim(), 15 + doc.getStringUnitWidth(`${key}: `) * 4, yPos)
        yPos += 6
      } else {
...
export const prepareReportData = (data: any, reportType: string): ReportData => {
  return {
    reportType,
