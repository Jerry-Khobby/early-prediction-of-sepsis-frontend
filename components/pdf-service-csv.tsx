...
export const generatePDF = async (
  ...
  if (reportData.safety_alerts) {
    yPos = addSafetyAlerts(
      doc,
      reportData.safety_alerts.precautions,
      yPos,
      font
    );
  }
...
function addRiskAssessment(
  ...
  const riskScore = riskAssessment?.score
    ? getRiskScore(riskAssessment?.score)
    : 0;
  const riskLevel =
  ...
  sortedFactors.forEach((factor) => {
    const description = factor.note || factor.marker;
...
function addModelExplanation(
  ...
  sortedFeatures.forEach((feature, index) => {
    ...
    doc.setFontSize(11);
    doc.setFont(font, "normal");
    doc.text(`${feature.marker}: ${percentage.toFixed(1)}%`, 15, yPos);
...
export const downloadPDFCsv = async (
  reportData: ReportData,
  options: PDFOptions,
...
export const emailPDFCsv = async (
  reportData: ReportData,
  options: PDFOptions,
  email: string
): Promise<boolean> => {
...
export const prepareReportDataCsv = (
  data: any,
  reportType: string
): ReportData => {
  return {
